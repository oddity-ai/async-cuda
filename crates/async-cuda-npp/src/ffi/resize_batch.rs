...
pub fn resize_batch(
    inputs_and_outputs: &mut [(
        ...
        &mut async_cuda_core::ffi::memory::DeviceBuffer2D<u8>,
    )],
    input_region: Region,
...
mod tests {
    ...
    fn test_resize_batch() {
        ...
        let mut inputs_and_outputs = (0..10)
            .map(|_| {
                ...
                let output = async_cuda_core::ffi::memory::DeviceBuffer2D::<u8>::new(3, 3, 3);
                (image, output)
            })
            .collect::<Vec<_>>();
    ...
    fn test_resize_batch_with_output_region() {
        ...
        let mut inputs_and_outputs = (0..10)
            .map(|_| {
                let image = to_device_2d!(&INPUT, 2, 2, 3, &context);
                let mut output = async_cuda_core::ffi::memory::DeviceBuffer2D::<u8>::new(2, 2, 3);
    ...
    fn test_it_panics_when_input_num_channels_incorrect() {
        let mut inputs_and_outputs = vec![
            (
                async_cuda_core::ffi::memory::DeviceBuffer2D::<u8>::new(100, 100, 2),
                async_cuda_core::ffi::memory::DeviceBuffer2D::<u8>::new(200, 200, 2),
            ),
            (
                async_cuda_core::ffi::memory::DeviceBuffer2D::<u8>::new(100, 100, 2),
                async_cuda_core::ffi::memory::DeviceBuffer2D::<u8>::new(200, 200, 2),
